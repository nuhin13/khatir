...
/**
 * Enhanced audit-log viewer client — EPIC-16.T-006 (replaces EPIC-11.T-011).
 *
 ...
 * filter set; the component owns filter + cursor state and threads them into the
 * presentational {@link AuditTable} (full actor/action/entity/date filters +
 * before/after diff). The CSV-export button reuses the *same* filters to build a
 * `?format=csv` download link (EPIC-16.T-002 stream), so legal exports honour
 * whatever the analyst has filtered down to. Cursor pagination keeps the
 * previous page visible while the next loads (`keepPreviousData`). Styling is
 * Notun Din tokens — no hardcoded hex/px.
 *
  ...
  return (
    <div className="space-y-s6">
      <div className="flex flex-wrap items-start justify-between gap-s4">
        ...
        <a
          href={auditCsvUrl(filters)}
          download="audit-log.csv"
          aria-label="Export filtered audit log as CSV"
          className="rounded-button bg-ink px-s5 py-s2 font-title text-sm font-semibold text-card hover:opacity-90"
        >
          Export CSV
        </a>
      </div>
      ...
      {isPending ? (
        ...
        <AuditTable
          ...
          onNext={() => setFilters((f) => ({ ...f, cursor: nextCursor }))}
          onPrevious={() => setFilters((f) => ({ ...f, cursor: prevCursor }))}
        />
      )}
    </div>
  );
